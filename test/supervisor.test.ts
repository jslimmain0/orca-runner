...
function cfg(port1: number, port2: number): Config {
  const base = { kind: 'command' as const, dir: process.cwd(), heapMb: 0, metaspaceMb: 0, cpus: 0, priority: 'normal' as const, jvmArgs: [], env: {} }
  return {
    services: [
...
describe('supervisor', () => {
      ...
      services: [
        {
          ...
          heapMb: 0, metaspaceMb: 0, cpus: 0, priority: 'normal', jvmArgs: [], env: {},
        },
      ],
    }
  ...
  it('BUILDING 중 stop()이 gradle 빌드 트리를 정리한다 (30초 안 기다림)', async () => {
    ...
    const cfg: Config = {
      services: [
        { name: 'slow-build', kind: 'spring', dir, port: 45828, heapMb: 512, metaspaceMb: 256, cpus: 2, priority: 'belowNormal', jvmArgs: [], env: {} },
      ],
    }
